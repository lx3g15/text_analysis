...
def parse_score(prediction_score):
    ...
    logit_prob = np.array([logit_prob[6617], logit_prob[6783], logit_prob[5526], logit_prob[6571]])
    logit_prob = tf.keras.layers.Softmax()(logit_prob).numpy()
    #print('logit_prob:', logit_prob)
    win_score = logit_prob[0] + logit_prob[2]
    loss_score = logit_prob[1] + logit_prob[3]
...
with open(input_data, 'r') as fin, open(output_data, 'w') as fout:
    for line in fin.readlines():
        ...
        fout.write('{}:{}\t{}:{}'.format(name1,first_person_final_score,name2,second_person_final_score))
